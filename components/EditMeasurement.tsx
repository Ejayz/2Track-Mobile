    ...
    <View className="items-center justify-center flex-1 bg-white">
      ...
      <ScrollView className="w-full bg-white">
        <Formik
          ...
          initialValues={{
            pallete_number: `${params.pallete_count}`,
            length: `${params.length}`,
            inside_diameter: `${params.inside_diameter}`,
            outside_diameter: `${params.outside_diameter}`,
            flat_crush: `${params.flat_crush}`,
            h20: `${params.h20}`,
          ...
          onSubmit={async (values, action) => {
            ...
            let response = await fetch(
              `${configurationData.api_url}/api/v1/edit_measurement?id=${route.params.id}`,
              {
                method: 'PUT',
                body: bodyContent,
                headers: headersList,
            ...
            if (response.ok) {
              action.resetForm();
              alert('Order fabrication removed successfully');
